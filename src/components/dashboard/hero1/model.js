...
export function toKpiCells(v) {
  ...
  return [
    c('powder', 'LIQUIDITÉ DISPO', { value: v.powder == null ? '—' : fmtUsdCompact(v.powder), usd: v.powder, money: true, est: true, sub: v.powderPct != null ? `${Math.round(v.powderPct)} % NLV déployable` : 'déployable', hint: 'Liquidité déployable (availableUsd cash-A) — vraie Buying Power/Excess Liquidity IBKR = 1re tâche d’implémentation (api/account-summary/sync.js)' }),
    c('day', 'DAY P&L', { value: v.dayPnl == null ? '—' : fmtUsdSigned(v.dayPnl), usd: v.dayPnl, money: true, tone: toneSign(v.dayPnl), sub: v.dayPct != null ? `${v.dayPct >= 0 ? '+' : '−'}${Math.abs(v.dayPct).toFixed(2)} % NLV` : null }),
    ...
    c('realized', 'REALIZED', { value: v.realized == null ? '—' : fmtUsdSigned(v.realized), usd: v.realized, money: true, tone: toneSign(v.realized), sub: v.ytd != null ? `YTD ${fmtUsdSigned(v.ytd)}` : 'cumulé' }),
    c('exposure', 'EXPOSURE', { value: v.exposure == null ? '—' : fmtUsdCompact(v.exposure), usd: v.exposure, money: true, sub: v.expoPct != null ? `${Math.round(v.expoPct)} % NLV · déployé` : 'déployé', hint: 'Capital DÉPLOYÉ (primes) = Max Loss long' }),

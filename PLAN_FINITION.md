...
- **U9 · Migration chrome legacy** — ✅ **FAIT (ce commit)**. Conversion **visuelle mécanique stricte** (zéro redesign, zéro changement de layout/logique/données) : Analytics + Journal + le sous-composant `RiskMetricsRow` (Analytics-only) passent du chrome legacy à la palette canonique flat, recette **copie locale namespacée par page** (comme `.history-page__panel`/`.history-page__kpi-tile` — aucun couplage CSS inter-pages).

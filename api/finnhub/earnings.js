// ═══════════════════════════════════════════════════════════════
//  VERCEL SERVERLESS — Finnhub earnings calendar proxy
//  Query params:
//    from  = YYYY-MM-DD (default: today)
//    to    = YYYY-MM-DD (default: today + 90 days)
//    symbol = optional ticker filter (server-side)
// ═══════════════════════════════════════════════════════════════
...
export default async function handler(req, res) {
  ...
  const to = DATE_RE.test(req.query.to || '') ? req.query.to : isoDate(90);
  ...
  try {
    ...
    let url = `https://finnhub.io/api/v1/calendar/earnings?from=${from}&to=${to}&token=${FINNHUB_KEY}`;
    ...
    // Cache 1 hour at edge — earnings dates are stable, heavy queries eat rate limit
    res.setHeader('Cache-Control', 's-maxage=3600, stale-while-revalidate=3600');
    return res.status(200).json(data);
  } catch (e) {
    return res.status(502).json({
      error: 'Finnhub earnings calendar indisponible',
      detail: e.message,
    });
  }
}
